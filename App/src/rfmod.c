...
//  uint8 cRcvBuf[64], cRcvLen;
  ...
  /*
  sp_disp_error("ret=%04x,cnt=%d", ret, p_ctx->reset_cpucard_count);
  if(ret == TRUE)
  {
    if(p_ctx->reset_cpucard_count == 0)
    {
      p_ctx->reset_cpucard_count++;
      sp_select_df03();
      sp_deselect_card();
      GetTime(&tm);
      sp_reset_time(p_ctx);
...
int sp_cpu_reset_time(sp_context* p_ctx)
{
  int ret = 0;
  SYSTIME tm;
  ret = sp_cpu_reset_card();
  ...
  ret = GetTime(&tm);
  sp_disp_error("GetTime,ret=%04x", ret);
  return ret;
}
