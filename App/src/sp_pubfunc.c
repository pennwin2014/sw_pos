...
void sp_copy_systime(sp_context* p_ctx, SYSTIME* tm)
{
  /*
    p_ctx->systime.chYear = tm->chYear;
    p_ctx->systime.chMonth = tm->chMonth;
    p_ctx->systime.chDay = tm->chDay;
    p_ctx->systime.chWeek = tm->chWeek;
    p_ctx->systime.chHour = tm->chHour;
    p_ctx->systime.chMinute = tm->chMinute;
    p_ctx->systime.chSecond = tm->chSecond;
    */
}
void sp_get_std_datetime(SYSTIME* p_tm, uint8 datetime[7])
{
  //sp_bcd_hex(const uint8 * bcd,uint8 bcd_len,char * hex)
}
...
static void uint8_to_bcd(const uint8 value, uint8* bcd)
{
  //大于100的就不能用一个字节了
  //assert(value<100);
...
void sp_uint16_to_bcd(const uint16 value, uint8 bcd[2])
{
  ...
  uint8_to_bcd(part2, &bcd[0]);
  uint8_to_bcd(part1, &bcd[1]);
}
void sp_uint8_to_bcd(const uint8 value, uint8* bcd)
{
  uint8_to_bcd(value, bcd);
}

void sp_get_time(sp_context* p_ctx)
{
  ...
  memcpy(p_ctx->current_datetime, current_datetime, 7);
  memcpy(p_ctx->today, p_ctx->current_datetime, 4);
}
...
int sp_check_context(sp_context* p_ctx)
{
  memset(p_ctx, 0, sizeof(sp_context));
  p_ctx->has_offline_rec = 0;
  p_ctx->rec_flash_full = false;

  sprintf(p_ctx->school_name, "上海大学");
  p_ctx->online_flag = false;
  p_ctx->reset_cpucard_count = 0;


  //param部分
  p_ctx->param.base_amt = 200;
  p_ctx->param.blacklist_index = 0;
  p_ctx->param.card_support = 0;
  p_ctx->param.consume_interval = 10;
  p_ctx->param.m1_alg = 1;
  p_ctx->param.offline_day = 0;
  p_ctx->param.record_index = 1;
  p_ctx->disp_cnt = 0;
  return 0;
}
