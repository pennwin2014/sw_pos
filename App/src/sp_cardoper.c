...
*/
static int do_calc_cpu_safe_mac(sp_context* p_ctx, sp_card* p_card,
...
static void do_cpu_parse_file10(sp_card* p_card, uint8* file10, uint8 len)
{
  ...
  /*
    char disp_msg[500];
    int i = 0;
    sp_disp_msg(100, "len=%d", len);
    memset(disp_msg, 0, 500);
    for(i = 0; i < len; i++)
    {
      sprintf(disp_msg + i * 3, "%02x-", file10[i]);
    }
...
static int do_cpu_pay_init(sp_context* p_ctx, sp_card* p_card, uint32 money)
{
  int ret;
  adpu_cmd_t cmd;
  ...
  cmd.send_len += 4;
  memcpy(cmd.send_buf + cmd.send_len, p_ctx->syspara.samno, 6);
  cmd.send_len += 6;
...
int sp_select_card_app(sp_context* ctx, sp_card* card)
{
  int ret = 0;
  adpu_cmd_t cmd;
  //选择df03
...
static int do_cpucard_read_card(sp_context* p_ctx, sp_card* p_card, uint8 read_level)
{
  int ret;
  adpu_cmd_t cmd;
  uint8 mac[4];
  ret = sp_select_card_app(p_ctx, p_card);
  if(ret)
  {
  ...
  if(read_level & SP_READ_FILE19)
  {
    ...
    //读file19文件
    //先进行外部认证
    ret = do_ext_auth(p_ctx, p_card->cardphyid, 1);
    if(ret)
    {
    ...
    cmd.send_len = 5;
    if(do_calc_cpu_safe_mac(p_ctx, p_card, cmd.send_buf, cmd.send_len, mac))
    {
      return SP_CALC_MAC_ERROR;
    }
  ...
  {
    //发一次消费初始化指令读取余额
    ret = do_cpu_pay_init(p_ctx, p_card, 0);
    if(ret)
    {
...
static int do_cpu_payment(sp_context* p_ctx, sp_card* p_card, uint32 money)
{
  int ret;
  uint8 mac1[4];
  adpu_cmd_t cmd;
  memset(&cmd, 0 , sizeof cmd);
  if(p_ctx->sam_aid != 0xdf03)
  {
    memcpy(cmd.send_buf, "\x00\xA4\x00\x00\x02\xDF\x03", 7);
    cmd.send_len = 7;
    ret = sp_sam_adpu_command(&cmd);
  ...
  /*trans datetime*/
  memcpy(cmd.send_buf + cmd.send_len, p_ctx->current_datetime, 7);
  cmd.send_len += 7;
  /* 消费密钥版本号*/
  //sp_disp_msg(12, "消费密钥版本号=%d", p_ctx->syspara.key_index);
  cmd.send_buf[cmd.send_len++] = p_ctx->syspara.key_index;
  /* 消费密钥算法标志*/
  cmd.send_buf[cmd.send_len++] = 0x00;
  /* 用户卡应用序列号*/
  ...
  if(ret)
  {
    sp_disp_error("Card debit4purchase error");
    return ret;
  }
  if(cmd.sw != ADPU_SUCCESS)
  {
  ...
  //写卡成功的tac
  memcpy(p_ctx->record.tac, cmd.recv_buf, 4);
  return 0;
}

static int do_cpu_get_prove(sp_context* p_ctx, sp_card* p_card, uint16 paycnt)
{
  adpu_cmd_t cmd;
  // char disp_msg[256];
  int ret = 0;
...
static int do_cpu_set_card_loss(sp_context* p_ctx, sp_card* p_card)
{
  ...
  cmd.send_len = 6;
  memcpy(cmd.send_buf + cmd.send_len, p_ctx->cardverno, 7);
  cmd.send_len += 7;
  if(do_calc_cpu_safe_mac(p_ctx, p_card, cmd.send_buf, cmd.send_len, mac))
  {
    return -1;
...
int sp_payinit(sp_context* p_ctx, sp_card* p_card, uint32 money)
{
  switch(p_card->cardtype)
  {
    case CPUCARD:
      return do_cpu_pay_init(p_ctx, p_card, money);
    case M1CARD:
      return 0;
    case SIMCARD:
      return 0;
...
int sp_payment(sp_context* p_ctx, sp_card* p_card, uint32 money)
{
  switch(p_card->cardtype)
  {
    case CPUCARD:
      return do_cpu_payment(p_ctx, p_card, money);
    case M1CARD:
      return 0;
    case SIMCARD:
      return 0;
...
int sp_get_prove(sp_context* p_ctx, sp_card* p_card, uint16 paycnt)
{
  switch(p_card->cardtype)
  {
    case CPUCARD:
      return do_cpu_get_prove(p_ctx, p_card, paycnt);
    case M1CARD:
      return 0;
    case SIMCARD:
      return 0;
...
int sp_set_card_loss(sp_context* p_ctx, sp_card* p_card)
{
  switch(p_card->cardtype)
  {
    case CPUCARD:
      return do_cpu_set_card_loss(p_ctx, p_card);
    case M1CARD:
      return 0;
    case SIMCARD:
      return 0;
...
int sp_halt(sp_context* p_ctx, sp_card* p_card)
{
  switch(p_card->cardtype)
  {
    case CPUCARD:
...
int sp_init_sam_card(sp_context* p_ctx, uint8 samno[6])
{
  int ret;
  ...
  if(ret)
  {
    sp_disp_msg(10, "SAM卡复位错误");
    return -1;
  }
  sp_disp_msg(10, "复位成功!!等待读取PSAM卡");
  ...
  if(ret)
  {
    sp_disp_msg(10, "读取PSAM卡信息错误,但继续。。");
    //return -1;
  }
  ...
  if(ret)
  {
    sp_disp_msg(10, "无法读取SAM号");
    return -1;
  }
  if(cmd.sw != ADPU_SUCCESS)
  {
    sp_disp_msg(90, "读取SAM卡号错误, sw=%04X", cmd.sw);
    return -1;
  }
  ...
  if(ret)
  {
    sp_disp_msg(100, "SAM卡应用目录错误");
    return -1;
  }
  if(cmd.sw != ADPU_SUCCESS)
  {
    sp_disp_msg(100, "Select sam df03 error,rw =%04X", cmd.sw);
    return -1;
  }
  p_ctx->sam_aid = 0xDf03;
  p_ctx->sam_ok = 1;
...
uint8 sp_do_write_card(sp_card* p_card, sp_context* p_ctx, sp_transdtl* ptransdtl)
{
  //写file10文件
  adpu_cmd_t cmd;
  int ret = 0;
  ...
  cmd.send_len = 5;
  //交易日期    4byte
  cmd.send_buf[cmd.send_len++] = 0x20;
  memcpy(cmd.send_buf + cmd.send_len, ptransdtl->last_datetime, 3);
  cmd.send_len += 3;
  //交易时间  3byte
  memcpy(cmd.send_buf + cmd.send_len, ptransdtl->last_datetime + 3, 3);
  cmd.send_len += 3;
  //  8-11  日累计交易金额  4 B
  sp_uint32_to_buffer(p_card->day_sum_amt, cmd.send_buf + cmd.send_len);
  //memcpy(cmd.send_buf + cmd.send_len, &(), 4);
  cmd.send_len += 4;
  //sp_disp_debug("write last_dev_no=[%02x%02x%02x%02x%02x%02x]", p_ctx->syspara.samno[0], p_ctx->syspara.samno[1],
  //p_ctx->syspara.samno[2], p_ctx->syspara.samno[3], p_ctx->syspara.samno[4], p_ctx->syspara.samno[5]);
  //12-17 上次交易终端号  6 b
  memcpy(cmd.send_buf + cmd.send_len, p_ctx->syspara.samno, 6);
  cmd.send_len += 6;
  //18-19 上次交易前次数  2 B
  uint16_to_pboc(ptransdtl->last_cnt, cmd.send_buf + cmd.send_len);
  cmd.send_len += 2;
  //20-23 上次交易金额  4
  sp_int_to_bigend(ptransdtl->last_amt, 4, cmd.send_buf + cmd.send_len);
  cmd.send_len += 4;
